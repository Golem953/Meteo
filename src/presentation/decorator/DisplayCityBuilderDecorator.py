...
class DisplayCityBuilderDecorator:
    ...
    def __init__(self, city_name: list[str], city_stations: list[str], city_station_provider: CityStationConfigMemoryProvider, city_builder: CityBuilder):
        ...
        for name, city in city_builder.items():
            ...
            for station in city.stations:
                ...
                for record in station.list_of_records:
                    print(f'    - Date: {record.paris_date}, Temp: {record.temperature.get_value()}°C, Humidity: {record.humidity.get_value()}%, Pression: {record.pressure.get_value()}Pa')